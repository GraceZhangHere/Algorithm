...
public class HuffmanCode {
  ...
  public static Tree getHuffmanTree(int[] counts) {
    ...
    Queue<Tree> heap = new PriorityQueue<>(); // Defined in Listing 24.10
    for (int i = 0; i < counts.length; i++) {
      if (counts[i] > 0)
        heap.offer(new Tree(counts[i], (char)i)); // A leaf node tree
    }
    
    while (heap.size() > 1) { 
      Tree t1 = heap.poll(); // Remove the smallest weight tree
      Tree t2 = heap.poll(); // Remove the next smallest weight 
      heap.offer(new Tree(t1, t2)); // Combine two trees
    }
